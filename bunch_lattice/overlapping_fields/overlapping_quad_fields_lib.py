...
from bunch_lattice.lattice import BunchAccNode, Drift, Quad
from bunch_lattice.lattice import AxisFieldRF_Gap
...
class EngeFunction(AbstractQuadFieldSourceFunction):
    ...
    def _getTrueEngeFunc(self, x):
        ...
        for i in range(1, len(self.a_arr)):
            ...
            x0 *= x
        if abs(sum_exp) > 30.0:
            sum_exp = 30.0 * sum_exp / abs(sum_exp)
    ...
    def _findCutOff(self, step, cutoff_level):
        ...
        self.normalization = 1.0
        init_val = self._getTrueEngeFunc(0.0)
        z = step
    ...
    def _normalize(self):
        ...
        for ind in range(self.n_func_points):
            ...
            self.func.add(z, val)
            sum_int += val
        sum_int -= (self._getTrueEngeFunc(0.0) + self._getTrueEngeFunc(step * (self.n_func_points - 1))) / 2.0
        sum_int *= 2.0 * step
...
class PMQ_Trace3D_Function(AbstractQuadFieldSourceFunction):
    ...
    def _findCutOff(self, step, cutoff_level):
        """Finds the distance from the center where the field is less than cutoff level"""
        init_val = self.getPMQ_FuncValue(0.0)
        z = step
    ...
    def _normalize(self):
        ...
        for ind in range(self.n_func_points):
            ...
            self.func.add(z, val)
            sum_int += val
        sum_int -= (self.getPMQ_FuncValue(0.0) + self.getPMQ_FuncValue(step * (self.n_func_points - 1))) / 2.0
        sum_int *= 2.0 * step
        self.normalization = 1.0 / sum_int
...
if __name__ == "__main__":
    ...
    for ind in range(2 * np - 1):
        ...
        print(" %12.5e  %12.5e " % (z * 1000.0, val))
