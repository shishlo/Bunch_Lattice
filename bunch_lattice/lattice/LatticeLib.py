...
class Lattice(AccLattice):
    ...
    def trackActions(self, actionsContainer, paramsDict={}, index_start=-1, index_stop=-1):
        ...
        if index_start < 0:
            index_start = 0
        if index_stop < 0:
            index_stop = len(self.getNodes()) - 1
        for node in self.getNodes()[index_start : index_stop + 1]:
            if paramsDict["stop tracking"]:
                break
...
class RF_Cavity(NamedObject, ParamsDictObject):
    ...
    def __init__(self, name="none"):
        ...
        self.__firstGapEntrancePhase = 0.0
        self.__firstGapEntranceDesignPhase = 0.0
        self.addParam("frequency", 0.0)
        self.addParam("phase", 0.0)
        self.addParam("amp", 1.0)
        self.addParam("designPhase", 0.0)
        self.addParam("designAmp", 1.0)
    ...
    def getAvgGapPhase(self):
        ...
        if len(self.__rfGaps) > 0:
            phase = phaseNearTargetPhase(self.__rfGaps[0].getGapPhase(), 0.0)
        for rfGap in self.__rfGaps:
            phase_new = phaseNearTargetPhase(rfGap.getGapPhase(), phase)
            avg_phase += phase_new
            phase = phase_new
        if len(self.__rfGaps) > 0:
            avg_phase /= len(self.__rfGaps)
        return avg_phase
    ...
    def reverseOrder(self):
        ...
        for rfGap in self.__rfGaps:
            ...
            gap_phase = phaseNearTargetPhase(gap_phase, 0.0)
